...
Cypress.Commands.add('calculateTwoDigits', (sign, answer, negative) => {
  ...
  if (negative) cy.get("[data-test-id='button-+/-']").click();
  cy.get(`[data-test-id='button-${sign}']`).click();
  cy.get("[data-test-id='button-3']").click();
  if (negative) cy.get("[data-test-id='button-+/-']").click();
  cy.get("[data-test-id='button-=']").click();
  cy.get('[data-test-id=display-value]').should('have.text', answer);
});

Cypress.Commands.add('calculateFractionalDigits', (sign, answer) => {
  cy.get("[data-test-id='button-9']").click();
  cy.get("[data-test-id='button-.']").click();
  cy.get("[data-test-id='button-9']").click().click().click();
  cy.get(`[data-test-id='button-${sign}']`).click();
  cy.get("[data-test-id='button-3']").click();
  cy.get("[data-test-id='button-.']").click();
  cy.get("[data-test-id='button-3']").click().click().click();
  cy.get("[data-test-id='button-=']").click();
  cy.get('[data-test-id=display-value]').should('have.text', answer);
});
...
Cypress.Commands.add('addHistory', (times) => {
  for (let i = 0; i < times; i++) cy.calculateDifficultExpression();
});
